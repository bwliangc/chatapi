...
func newContentModerationKeywordMatcher(keywords []string) *contentModerationKeywordMatcher {
	...
	for keywordIndex, keyword := range keywords {
		...
		lowerKeywords[keywordIndex] = lowerKeyword
		state := int32(0)
...
func (m *contentModerationKeywordMatcher) Match(text string) (string, bool) {
	...
	for index := 0; index < len(lower); index++ {
		...
		for candidateState := state; candidateState != 0; candidateState = m.nodes[candidateState].failure {
			candidate := m.nodes[candidateState].bestKeyword
			if candidate < 0 || int(candidate) >= len(m.keywords) {
				continue
			}
			lowerKeyword := m.lowerKeywords[candidate]
			end := index + 1
			start := end - len(lowerKeyword)
			if start < 0 || !isBlockedKeywordBoundary(lower[:start], lower[end:], lowerKeyword) {
				continue
			}
			bestKeyword = minKeywordIndex(bestKeyword, candidate)
		}
		if bestKeyword == 0 {
