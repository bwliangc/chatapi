...
func OpenAICodexTicketStatuses(account *Account, cfg config.OpenAICodexTicketConfig, now time.Time) []OpenAICodexTicketStatus {
	...
	for _, model := range models {
		...
		status.Blocked = cfg.Enabled && CodexTicketHarvestEnabled(account, model) && !OpenAICodexAllowsWithoutTicket(account, !cfg.FailClosed) && !status.Ready
...
func (t *openAICodexTicket) usable(now time.Time, targetLen int, allowExpired bool, reuseWindow time.Duration) bool {
	if !t.structurallyValid(targetLen) || t.ExpiresAt.IsZero() {
		return false
	}
	if !t.expired(now) {
		return true
...
func (s *OpenAIGatewayService) applyOpenAICodexTicket(ctx context.Context, account *Account, model string, h http.Header) error {
	if s == nil || h == nil || !CodexTicketHarvestEnabled(account, normalizeOpenAICodexTicketModel(model)) || !s.openAICodexTicketEnabledContext(ctx) {
		return nil
	}
	model = normalizeOpenAICodexTicketModel(model)
	if model == "" || !CodexTicketHarvestEnabled(account, model) || !s.openAICodexTicketGatedModel(model) {
		return nil
	}
...
func (s *OpenAIGatewayService) openAICodexTicketBlocksAccount(account *Account, outboundModel string) bool {
	if s == nil || !CodexTicketHarvestEnabled(account, normalizeOpenAICodexTicketModel(outboundModel)) || !s.openAICodexTicketEnabled() {
		return false
	}
	cfg := s.openAICodexTicketConfig()
	...
	if !CodexTicketHarvestEnabled(account, model) || !s.openAICodexTicketGatedModel(model) {
		return false
	}
	ticket := s.lookupOpenAICodexTicket(account, model)
...
func (s *OpenAIGatewayService) fireOpenAICodexTicketProbe(ctx context.Context, account *Account, token, model, proxyURL string, attemptTimeout time.Duration) (state string, cookie string, status int, err error) {
	...
	body := []byte(`{"model":` + codexTicketJSONString(model) + `,"store":false,"stream":true,"instructions":"Reply with exactly: pong","input":[{"role":"user","content":[{"type":"input_text","text":"ping"}]}]}`)
...
func codexTicketJSONString(v string) string {
...
// cadence from stretching a requested 30-40 second retry beyond its upper bound.
...
// refreshOpenAICodexTickets probes opted-in account/models according to their retry
// schedule, with bounded concurrency. The next scan starts after all probes finish.
func (s *OpenAIGatewayService) refreshOpenAICodexTickets(ctx context.Context) {
	...
	for i := range accounts {
		account := accounts[i]
		if !isOpenAICodexTicketAccount(&account) || account.Status != StatusActive || account.Extra[codexTicketAccountEnabledKey] != true {
			continue
		}
