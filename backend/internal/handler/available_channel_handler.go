...
// userGroupInfo 用户可见的「公开分组信息」条目。
//
// 在 userAvailableGroup 的白名单字段基础上，额外带分组描述与该分组可用模型列表。
// 模型来自该分组关联渠道的 SupportedModels 并集，并按分组自身 platform 做了脱敏。
...
// ListAvailableGroupsInfo 列出当前用户可见分组的公开信息（费率 / 平台 / 模型）。
// GET /api/v1/groups/available/info
//
// 数据来源：用户可访问分组（GetAvailableGroups）∩ 渠道关联分组（ListAvailable），
// 把渠道维度的 SupportedModels 反转聚合到分组维度，按分组自身 platform 脱敏并按
// 模型名去重。复用 available-channels 开关：未启用时返回空数组。
func (h *AvailableChannelHandler) ListAvailableGroupsInfo(c *gin.Context) {
	subject, ok := middleware.GetAuthSubjectFromContext(c)
	if !ok {
		response.Unauthorized(c, "User not authenticated")
		return
	...
	response.Success(c, buildGroupsInfo(userGroups, channels))
}

// buildGroupsInfo 把渠道维度的支持模型反转聚合到分组维度（纯函数，便于单测）。
//
//   - 仅保留 userGroups（用户可访问分组）；
//   - 每个分组的模型 = 关联的 Active 渠道 SupportedModels 并集，按分组自身 platform
//     脱敏（防跨平台泄漏），并按模型名去重；
//   - 输出按 platform、再按 name 稳定排序，模型内部也按 name 排序，保证可复现。
func buildGroupsInfo(
	userGroups []service.Group,
	channels []service.AvailableChannel,
) []userGroupInfo {
	allowedGroupIDs := make(map[int64]struct{}, len(userGroups))
	groupPlatform := make(map[int64]string, len(userGroups))
	for i := range userGroups {
		allowedGroupIDs[userGroups[i].ID] = struct{}{}
		groupPlatform[userGroups[i].ID] = userGroups[i].Platform
	}

	modelsByGroup := make(map[int64][]userSupportedModel, len(userGroups))
	seenByGroup := make(map[int64]map[string]struct{}, len(userGroups))
	for _, ch := range channels {
		if ch.Status != service.StatusActive {
			continue
		}
		for _, g := range ch.Groups {
			...
			platformSet := map[string]struct{}{groupPlatform[g.ID]: {}}
			for _, m := range toUserSupportedModels(ch.SupportedModels, platformSet) {
				seen := seenByGroup[g.ID]
				if seen == nil {
					seen = make(map[string]struct{})
					seenByGroup[g.ID] = seen
				}
				if _, dup := seen[m.Name]; dup {
					continue
				}
				seen[m.Name] = struct{}{}
				modelsByGroup[g.ID] = append(modelsByGroup[g.ID], m)
			}
		}
	}

	out := make([]userGroupInfo, 0, len(userGroups))
	for i := range userGroups {
		g := &userGroups[i]
	...
	sort.SliceStable(out, func(a, b int) bool {
		if out[a].Platform != out[b].Platform {
