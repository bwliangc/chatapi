...
func (h *DashboardHandler) GetModelStats(c *gin.Context) {
	...
	stats, hit, err := h.getModelStatsCached(c.Request.Context(), startTime, endTime, userID, apiKeyID, accountID, groupID, modelSource, requestType, stream, nativeCompactionV2, billingType, excludeAdmins, upstreamModelMismatch)
	if err != nil {
...
func (h *DashboardHandler) GetGroupStats(c *gin.Context) {
	...
	stats, hit, err := h.getGroupStatsCached(c.Request.Context(), startTime, endTime, userID, apiKeyID, accountID, groupID, requestType, stream, nativeCompactionV2, billingType, excludeAdmins, upstreamModelMismatch)
	if err != nil {
...
func (h *DashboardHandler) GetUserBreakdown(c *gin.Context) {
	...
	if !ok {
		response.BadRequest(c, "Invalid sort_by, use actual_cost/tokens/input_tokens/output_tokens/cache_tokens/requests/cost/account_cost")
		return
	}
	dim.SortBy = sortBy
