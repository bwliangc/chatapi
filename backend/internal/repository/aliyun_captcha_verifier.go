...
func normalizeAliyunCaptchaError(err error) error {
	...
	if errors.As(err, &teaErr) && isAliyunAPIErrorCode(tea.StringValue(teaErr.Code)) {
	...
	if errors.As(err, &daraErr) && isAliyunAPIErrorCode(dara.StringValue(daraErr.Code)) {
		return &service.AliyunCaptchaAPIError{
...
func isAliyunAPIErrorCode(code string) bool {
