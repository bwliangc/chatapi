...
func TestUsageLogRepositoryGetUserSpendingRanking(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{"user_id", "email", "username", "actual_cost", "requests", "tokens", "total_actual_cost", "total_requests", "total_tokens"}).
		AddRow(int64(2), "beta@example.com", "beta", 12.5, int64(8), int64(900), 41.75, int64(32), int64(2800)).
		AddRow(int64(4), "delta@example.com", "delta", 12.5, int64(10), int64(800), 41.75, int64(32), int64(2800)).
		AddRow(int64(1), "alpha@example.com", "alpha", 12.5, int64(9), int64(800), 41.75, int64(32), int64(2800)).
		AddRow(int64(3), "gamma@example.com", "", 4.25, int64(5), int64(300), 41.75, int64(32), int64(2800))

	mock.ExpectQuery("WITH user_spend AS \\(.*ORDER BY actual_cost DESC, tokens DESC, requests DESC, user_id ASC LIMIT \\$3.*ORDER BY actual_cost DESC, tokens DESC, requests DESC, user_id ASC").
	...
	require.Equal(t, &usagestats.UserSpendingRankingResponse{
		Ranking: []usagestats.UserSpendingRankingItem{
			{UserID: 2, Email: "beta@example.com", Username: "beta", ActualCost: 12.5, Requests: 8, Tokens: 900},
			{UserID: 4, Email: "delta@example.com", Username: "delta", ActualCost: 12.5, Requests: 10, Tokens: 800},
			{UserID: 1, Email: "alpha@example.com", Username: "alpha", ActualCost: 12.5, Requests: 9, Tokens: 800},
			{UserID: 3, Email: "gamma@example.com", ActualCost: 4.25, Requests: 5, Tokens: 300},
		},
		TotalActualCost: 41.75,
		TotalRequests:   32,
		TotalTokens:     2800,
	}, got)
	require.NoError(t, mock.ExpectationsWereMet())
...
func TestResolveUserBreakdownOrderByActualCost(t *testing.T) {
	require.Equal(t,
		"ORDER BY actual_cost DESC, total_tokens DESC, requests DESC, user_id ASC",
		resolveUserBreakdownOrderBy("actual_cost"),
	)
}
