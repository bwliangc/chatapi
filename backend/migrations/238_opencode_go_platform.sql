...
-- 新约束是 237 的超集，必须同时保留 MiniMax 和 custom 分支的自定义平台。
...
BEGIN
    ...
    IF monitor_constraint_def IS NULL OR position('opencode_go' IN monitor_constraint_def) = 0
       OR position('custom' IN monitor_constraint_def) = 0 THEN
    ...
    IF template_constraint_def IS NULL OR position('opencode_go' IN template_constraint_def) = 0
       OR position('custom' IN template_constraint_def) = 0 THEN
