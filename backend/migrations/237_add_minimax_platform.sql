...
--
-- custom 分支保留自定义平台，避免升级时拒绝已有 custom 记录。
...
BEGIN
    ...
    IF monitor_constraint_def IS NULL OR position('minimax' IN monitor_constraint_def) = 0
    ...
    IF template_constraint_def IS NULL OR position('minimax' IN template_constraint_def) = 0
