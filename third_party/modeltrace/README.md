...
目前支持 OpenAI / Anthropic 的 API Key 和 OAuth 文本账号，不支持影子、Bedrock、Vertex 或合成 UI 账号。所有采样固定同一账号，复用代理、认证和模型映射，不经过账号池调度；存在 OpenAI 传输插件时仍遵循该账号现有传输路径。每次采样重新检查开关、账号可调度状态和模型映射，每个正在执行的采样分别占用一个正常账号并发槽。关闭功能后阻止新任务和后续采样，当前上游调用仍受 90 秒时限约束。

目标三份有效回答，默认最多三路同时采集；账号并发上限为 1/2 时自动降到 1/2 路，未设限时仍最多三路。只为缺失的有效样本补采，总计最多六次，单次 90 秒、总计 10 分钟。页面显示已发起次数、正在进行的请求数及有效回答数。网络错误/上游错误/截断立即失败并取消其余请求；用户取消或离开页面也会取消全部请求，释放并发槽后才解除该账号的检测任务锁。不自动切换账号或重试网络错误；正常业务已占满账号并发额度时会提示并发已满。API Key 请求使用最多 4096 输出 token；Codex OAuth 不接受此参数，使用时间、响应字节上限控制，因此不能承诺固定 token 消耗上限。OAuth 必需的客户端身份前缀和模型规范化会保留，可能影响指纹分布。
