...
export async function getAutoReset(id: number): Promise<AccountAutoResetSettings> {
  const { data } = await apiClient.get<AccountAutoResetSettings>(`/admin/accounts/${id}/auto-reset`)
  return data
}
