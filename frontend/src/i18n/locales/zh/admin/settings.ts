    ...
    settings: {
      ...
      features: {
        ...
        leaderboardReward: {
          ...
          displayTopN: '榜单展示名额（前 N 名）',
          displayTopNHint: '控制用户可查看的排行榜总名额，默认展示前 10 名；超过 10 名时分页显示。与奖励名额相互独立。',
      ...
      gatewayForwarding: {
        ...
        codexTimezoneRewrite: 'Codex 时区重写',
        codexTimezoneRewriteHint: '将 Codex 请求体 environment_context 中的时区和当前日期改写为配置的目标时区。',
        codexTimezone: 'Codex 目标时区',
        codexTimezonePlaceholder: '留空使用服务器应用时区，例如 America/Los_Angeles',
        codexTimezoneSearchPlaceholder: '搜索 IANA 时区',
        codexTimezoneServerOption: '服务器应用时区（留空）',
        codexTimezoneHint: '填写 IANA 时区名称，例如 Asia/Shanghai 或 America/Los_Angeles；留空时使用服务器应用时区。',
