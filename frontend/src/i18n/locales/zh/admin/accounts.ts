export default {
    accounts: {
      ...
      setPrivacy: '设置隐私',
      abnormalNotification: '异常通知',
      abnormalNotificationHint: '配置账号异常时的邮件通知',
      abnormalNotificationSettings: '异常通知设置',
      abnormalNotificationEnabled: '邮件通知',
      abnormalNotificationEnabledHint: '账号进入勾选的状态时自动发送邮件',
      abnormalNotificationStatuses: '通知状态',
      abnormalNotificationStatusRequired: '请至少勾选一种通知状态',
      ...
      abnormalNotificationStatusHints: {
        error: '认证失效、账号错误等需要处理的异常',
        rate_limited: '包括 5 小时或周额度耗尽；模型限流按模型分别通知',
        temp_unschedulable: '临时错误、连续失败或调度阈值导致暂停调度',
        overloaded: '上游服务过载，账号进入冷却期',
      },
      abnormalNotificationEmail: '收件邮箱',
      abnormalNotificationEmailPlaceholder: '请输入收件邮箱',
      abnormalNotificationEmailHint: '每种状态在持续期间只通知一次，恢复后再次触发会重新通知。',
      abnormalNotificationSaved: '异常通知设置已保存',
      abnormalNotificationLoadFailed: '加载异常通知设置失败',
      abnormalNotificationSaveFailed: '保存异常通知设置失败',
      autoReset: '自动重置',
      autoResetHint: '配置 OpenAI 重置次数的自动使用策略',
      autoResetSettings: '自动重置设置',
      autoResetEnabled: '自动使用重置次数',
      autoResetEnabledHint: '任一自定义条件满足时，自动使用一次重置次数',
      autoResetConditions: '触发条件',
      autoResetConditionsHint: '已添加的条件按“或”执行，任一条件满足即可触发。',
      autoResetAddCondition: '添加{condition}',
      autoResetRemoveCondition: '删除条件',
      autoResetNoConditions: '尚未添加触发条件',
      autoResetWeeklyThreshold: '周限额使用率',
      autoResetWeeklyThresholdHint: '7 天窗口达到此使用率后执行一次重置；使用率回落后才会再次触发。',
      autoResetExpiryMinutes: '过期前触发',
      autoResetExpiryMinutesHint: '最早到期的可用重置次数进入此分钟范围后执行重置。',
      autoResetEmail: '通知邮箱',
      autoResetEmailPlaceholder: '请输入通知邮箱',
      autoResetEmailHint: '每次自动重置成功后发送邮件。',
      autoResetSaved: '自动重置设置已保存',
      autoResetLoadFailed: '加载自动重置设置失败',
      autoResetSaveFailed: '保存自动重置设置失败',
      minutes: '分钟',
