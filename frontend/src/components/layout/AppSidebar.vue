...
const flagSubscriptionManagement = makeSidebarFlag(FeatureFlags.subscriptionManagement)
const flagModelDetection = makeSidebarFlag(FeatureFlags.modelDetection)
...
//
// 条目顺序：在线使用 → 密钥 → 用量 → 可用渠道 → 渠道状态 → 订阅/支付 → 兑换/资料。
// 在线使用置于首位；可用渠道紧挨渠道状态之上，让用户"先看自己能用什么、再看对应状态"。
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  const items: NavItem[] = []
  // 在线使用置于菜单首位
  items.push({ path: '/playground', label: t('nav.onlinePlayground'), icon: ChatIcon, hideInSimpleMode: true, featureFlag: flagOnlinePlayground })
  if (withDashboard) {
  ...
  items.push(
    ...
    { path: '/subscriptions', label: t('nav.mySubscriptions'), icon: CreditCardIcon, hideInSimpleMode: true, featureFlag: () => flagSubscription() && flagSubscriptionManagement() },
    ...
    { path: '/leaderboard', label: t('nav.leaderboard'), icon: ChartIcon, hideInSimpleMode: true, featureFlag: flagLeaderboardRanking },
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/subscriptions', label: t('nav.subscriptions'), icon: CreditCardIcon, hideInSimpleMode: true, featureFlag: () => flagSubscription() && flagSubscriptionManagement() },
    { path: '/admin/accounts', label: t('nav.accounts'), icon: GlobeIcon },
    { path: '/admin/model-detection', label: t('nav.modelDetection'), icon: SignalIcon, featureFlag: flagModelDetection },
