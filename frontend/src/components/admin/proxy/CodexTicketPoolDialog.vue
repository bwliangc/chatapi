<template>
  <BaseDialog :show="show" :title="t('admin.accounts.codexTicket.poolTitle')" width="normal" @close="$emit('close')">
    ...
    <template #footer><button class="btn btn-secondary" @click="$emit('close')">{{ t('common.cancel') }}</button><button class="btn btn-primary" :disabled="!loaded || loading || saving || (mode === 'custom' && !selected.length)" @click="save">{{ saving ? t('admin.accounts.codexTicket.saving') : t('common.save') }}</button></template>
  </BaseDialog>
</template>
