...
describe('subscription route guard (opt-out flag)', () => {
  ...
  it.each([
    ...
    [true, true, undefined],
  ])('respects both subscription switches (%s, %s)', async (subscription, management, target) => {
    appStore.cachedPublicSettings = {
      subscription_enabled: subscription,
      subscription_management_enabled: management,
    }
    const { navigation, next } = runGuard(
      { requiresSubscription: true, requiresSubscriptionManagement: true },
      '/subscriptions',
    )
    await navigation
    expect(next).toHaveBeenCalledOnce()
