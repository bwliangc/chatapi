        ...
        <div v-show="activeTab === 'gateway'" class="space-y-6">
          ...
          <div class="card">
            ...
            <div class="p-6 space-y-4">
                ...
                <div class="flex items-center justify-between gap-4">
                  <div class="min-w-0">
                    <h3 class="text-base font-semibold text-gray-900 dark:text-white">{{ t('admin.settings.gatewayForwarding.codexTicketTTL') }}</h3>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ t('admin.settings.gatewayForwarding.codexTicketTTLDesc') }}</p>
                  </div>
                  <input id="codex-ticket-ttl" v-model.number="form.openai_codex_ticket_ttl_seconds" :aria-label="t('admin.settings.gatewayForwarding.codexTicketTTL')" type="number" min="60" max="86400" step="1" class="input w-32 text-sm" />
                </div>
                <div class="flex items-center justify-between gap-4">
                  <div class="min-w-0">
                ...
                <div class="flex items-center justify-between gap-4">
                  ...
                  <input id="codex-ticket-reuse-window" v-model.number="form.openai_codex_ticket_reuse_expired_max_seconds" :aria-label="t('admin.settings.gatewayForwarding.codexTicketReuseExpiredMax')" type="number" min="0" max="86400" step="1" class="input w-32 text-sm" />
                </div>
                <div>
                  <h3 class="text-base font-semibold text-gray-900 dark:text-white">
        ...
        <div class="card">
          ...
          <div class="space-y-5 p-6">
            <div class="flex items-center justify-between">
              ...
              <Toggle v-model="form.online_playground_enabled" />
            </div>
          </div>
        </div>

        <!-- Subscription Management (订阅功能) feature card -->
        <div class="card">
          <div class="border-b border-gray-100 px-6 py-4 dark:border-dark-700">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
              {{ t('admin.settings.features.subscriptionManagement.title') }}
            </h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
              {{ t('admin.settings.features.subscriptionManagement.description') }}
            </p>
          </div>
          <div class="space-y-5 p-6">
            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
                  {{ t('admin.settings.features.subscriptionManagement.enabled') }}
                </label>
                <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                  {{ t('admin.settings.features.subscriptionManagement.enabledHint') }}
                </p>
              </div>
              <Toggle v-model="form.subscription_management_enabled" />
            </div>
          </div>
        </div>

        <!-- Model Plaza feature card -->
        <div class="card">
          <div class="border-b border-gray-100 px-6 py-4 dark:border-dark-700">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
...
async function saveSettings() {
  ...
  try {
    ...
    const payload: UpdateSettingsRequest = {
      ...
      openai_codex_ticket_ttl_seconds: Math.min(
        86400,
        Math.max(
          60,
          Math.round(Number(form.openai_codex_ticket_ttl_seconds) || 200),
        ),
      ),
      ...
      subscription_management_enabled: form.subscription_management_enabled,
